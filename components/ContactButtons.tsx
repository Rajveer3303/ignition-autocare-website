...
/**
 * Call + WhatsApp pair. Shared so the hero and in-page CTAs stay identical and
 * each page can pre-fill its own WhatsApp message.
 */
export default function ContactButtons({
  ...
  return (
    <div className={`flex flex-col gap-3 sm:flex-row ${className}`}>
      <a
        href={SITE.phoneHref}
        className={`flex items-center justify-center gap-2 rounded-full bg-brand-600 font-bold text-white shadow transition hover:bg-brand-500 ${pad}`}
      >
        <svg viewBox="0 0 24 24" width={icon} height={icon} fill="white" aria-hidden="true">
      ...
      <a
        ...
        className={`flex items-center justify-center gap-2 rounded-full font-bold text-white shadow transition hover:opacity-90 ${pad}`}
        style={{ backgroundColor: "#25D366" }}
      >
        <svg viewBox="0 0 32 32" width={icon} height={icon} fill="white" aria-hidden="true">
